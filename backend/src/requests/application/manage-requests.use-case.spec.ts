...
const fakeEvents = (): RequestEventRepository => ({
  append: jest.fn(
  ...
  listByRequest: jest.fn(async () => []),
});
const emitter = { emit: jest.fn() } as unknown as EventEmitter2;
const makeUseCase = (repo: RequestRepository) =>
  new ManageRequestsUseCase(repo, fakeEvents(), emitter);

describe('ManageRequestsUseCase', () => {
  it('lists only own requests for a requester, all for a reviewer', async () => {
    const uc = makeUseCase(new FakeRepo([req('a', 'u1'), req('b', 'u9')]));
  ...
  it('creates a draft owned by the caller', async () => {
    const uc = makeUseCase(new FakeRepo());
  ...
  it('rejects updating a foreign request', async () => {
    const uc = makeUseCase(new FakeRepo([req('a', 'u9')]));
    await expect(
  ...
  it('submits an own draft', async () => {
    const uc = makeUseCase(new FakeRepo([req('a', 'u1')]));
